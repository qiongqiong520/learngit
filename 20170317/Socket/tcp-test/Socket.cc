...
namespace wd
{
	...
	void Socket::bindAddress(const InetAddress &addr)
	{
		...
		{
 	 		fprintf(stderr,"bind address errno\n");
			exit(EXIT_FAILURE);
		}
	}
	...
	void Socket::listen()
	{
		...
		{
	 		fprintf(stderr,"listen address errno\n");
			exit(EXIT_FAILURE);
		}
	}
	...
	{
	 	int optval=on?1:0;
		if(::setsockopt(sockfd_,SOL_SOCKET,SO_REUSEADDR,&optval,static_cast<socklen_t>(sizeof optval))==-1)
		{
