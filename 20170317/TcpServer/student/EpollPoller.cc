...
namespace {
	...
	void addEpollReadFd(int epollfd,int fd)
	{
		...
		{
			char buf[1024];
			ssize_t nread =recvbuf(sockfd,buf,sizeof buf);
			if(nread ==-1)
			{
				perror("recvbuf");
				exit(EXIT_FAILURE);
...
namespace wd
{
	...
	void EpollPoller::waitEpollFd()
	{
		int nready;
		do{
			nready =::epoll_wait(epollfd_,&*events_.begin(),
	...
	void EpollPoller::handleMessage(int peerfd)
	{
		...
		if(isClosed)
		{
			//调用conn的close时间handleCloseCallback
